...
function ATMG:updateGradInput(_tranformParams, _gradParams)
  ...
  scale_cord= transformParams[{{},{2,3}}]:clone()
  scale_cord[{{},1}]:cmul(transformParams[{{},1}])	
  scale_cord[{{},2}]:cmul(transformParams[{{},1}])
  -- find the indexes for which spatial transformer (tx,ty) have gone outside image boundary  	
  out_ind1= torch.abs(scale_cord[{{},1}]):gt(1):double():nonzero()
  out_ind2= torch.abs(scale_cord[{{},2}]):gt(1):double():nonzero()
  
  scale_sq= torch.cmul(transformParams[{{},1}],transformParams[{{},1}]);
  y_sq= torch.cmul(transformParams[{{},2}],transformParams[{{},2}]);
  x_sq= torch.cmul(transformParams[{{},3}],transformParams[{{},3}]);
  ...
  -- compute the gradient which will be used if spatial transformer (tx,ty) have gone outside image boundary 
  new_grad = self.gradInput:clone()
  new_grad[{{},1}]=torch.cmul(x_sq,transformParams[{{},1}])+torch.cmul(y_sq,transformParams[{{},1}])
  new_grad[{{},2}]= torch.cmul(transformParams[{{},2}],scale_sq)
  new_grad[{{},3}]= torch.cmul(transformParams[{{},3}],scale_sq)
  if(out_ind1:nDimension()>0) then 
  	self.gradInput[{out_ind1,{}}]=new_grad[{out_ind1,{}}] 
  end
  if(out_ind2:nDimension()>0) then 
  	self.gradInput[{out_ind2,{}}]=new_grad[{out_ind2,{}}] 
  end
